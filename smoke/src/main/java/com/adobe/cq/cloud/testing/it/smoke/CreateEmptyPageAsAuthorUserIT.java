...
public class CreateEmptyPageAsAuthorUserIT {
    ...
    public EmptyPage emptyPageRule = new EmptyPage(userRule.getClientSupplier());

    @Rule
    public TestRule cqRuleChain = RuleChain.outerRule(cqBaseRule).around(userRule).around(emptyPageRule);
    ...
    public void testCreatePageAsAuthor() throws ClientException, InterruptedException {
        ...
        Assert.assertTrue(cqBaseClassRule.authorRule.getAdminClient().adaptTo(CQClient.class)
                .pageExistsWithRetry(emptyPageRule.getPath(), 20000));
        // This shows that it exists for the author user in a different way
        userRule.getClient().getAuthorSitesPage(emptyPageRule.getPath(), 200);
    }
